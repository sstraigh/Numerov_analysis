...
int main(int argc, char * argv[]){
    ...
    if (argc != 1){
	std::cerr<<"Usage: ./Numerov-Analysis (no command line input, read from INPUT.dat)\n";
	return 1;
    }
    ...
    std::cerr << num_energy_levels << '\t' <<  mass << '\t' <<  x_min << '\t' <<  x_max << '\t' <<  total_time << std::endl;
...
void propagate_computation(int num_energy_levels, long double mass, long double x_min,
    ...
    std::string NB_mu_filename = "dipole_surface.dat";
