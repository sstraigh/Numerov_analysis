...
void propagate_computation(int num_energy_levels, long double mass, long double x_min,
    ...
    std::string output_freq_filename = "freq_file_1B.dat";
    std::string output_tdip_filename = "transition_dipole_1B.dat";

    std::string pot_filename = "STATIS_POT";
    std::string coord_filename = "coord";
    std::string inddip_filename = "ind_dip";
    std::string perdip_filename = "perm_dip";
    std::string NB_mu_filename = "NB-dipole";
...
//	std::cout<<"Reading in the frame"<<std::endl;
...
//	std::cout<<"Frame Read In"<<std::endl;
	std::string blank_line;
	//reset the string buffers to capture the next (read blank) line
	old_time = current_time;

//	for (int i=0; i<num_points; ++i) std::cout<<potential_at_scan[i]<<std::endl;

//	std::cout<<potential_at_scan[7]<<std::endl;
...
//		std::cout<<"here 4 "<<wavefunction_found<<std::endl;
